...
export default function AIRouter() {
  ...
  const [state, setState] = useState('idle'); // idle | loading | result | crisis | error | unmatched
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (data.unmatched) {
        setUnmatchedNote(data.suggestion || '');
        setState('unmatched');
        return;
      }
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                disabled={!input.trim() || input.trim().length < 3}
                style={{
                  background: input.trim().length >= 3 ? 'var(--blue)' : 'rgba(31,42,55,0.08)',
                  color: input.trim().length >= 3 ? 'white' : 'var(--pewter)',
                  border: 'none', borderRadius: 100,
                  padding: '10px 24px', fontFamily: 'var(--sans)',
                  fontSize: 15, fontWeight: 600, cursor: input.trim().length >= 3 ? 'pointer' : 'default',
      ...
      {state === 'crisis' && (
        <div style={{
          background: 'white', borderRadius: 16,
          border: '1px solid rgba(201,123,106,0.3)',
          padding: 24,
        }}>
          <h3 style={{
      ...
      {state === 'unmatched' && (
        <div style={{
          background: 'var(--mist)', borderRadius: 16, padding: 24,
        }}>
          <h3 style={{
            fontFamily: 'var(--serif)', fontSize: 20, fontWeight: 400,
            color: 'var(--slate)', marginBottom: 12,
